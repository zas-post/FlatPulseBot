        ...
        try:
            ...
            for card in cards:
                try:
                    ...
                    link_element = card.find('a', class_=lambda c: c and 'header' in c) or card.find('a')
                    if not link_element or 'href' not in link_element.attrs:
                        continue

                    href = link_element['href']
                    # Исправляем относительные ссылки, если они есть
                    item_url = href if href.startswith('http') else f"https://spb.cian.ru{href}"

                    # Заголовок (например, "2-комн. кв., 56 м²")
                    title_element = card.find('span', data_mark='OfferTitle') or link_element
                    title = title_element.get_text(strip=True) if title_element else "Квартира на Циан"

                    # 2. Поиск цены
                    price_element = card.find('span', data_mark='MainPrice') or card.find('span', class_=lambda c: c and 'price' in c)
                    ...
                    logging.debug(f"[Cyan Scraper] Ошибка разбора отдельной карточки: {card_error}")
                    continue
