...
class BaseScraper:
    ...
    def get_clean_options(self):
        ...
        # 🔥 Изолируем порты отладки, чтобы потоки Авито и Циана не конфликтовали
    ...
    def get_driver(self):
        """Возвращает новый экземпляр драйвера с жестко зафиксированной версией Chrome"""
        try:
            current_options = self.get_clean_options()

            # 🔥 Фиксируем мажорную версию ChromeDriver под версию Chrome в Docker (150)
            driver = uc.Chrome(
                options=current_options,
                startup_timeout=35,
                version_main=150  # <--- КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: принудительно используем 150-й драйвер
            )
